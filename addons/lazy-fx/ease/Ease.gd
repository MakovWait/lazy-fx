...
# Sine
static func easeInSine(x: float, offset: float=0, length: float=1) -> float:
   x -= offset
   x /= length
   return (0.0 if x < 0 else (1.0 if x > 1.0 else 1.0 - cos((x * PI) / 2.0)))

static func easeOutSine(x: float, offset: float=0, length: float=1) -> float:
   x -= offset
   x /= length
   return (0.0 if x < 0 else (1.0 if x > 1.0 else sin((x * PI) / 2.0)))
   
static func easeInOutSine(x: float, offset: float=0, length: float=1) -> float:
   x -= offset
   x /= length
   return (0.0 if x < 0 else (1.0 if x > 1.0 else -(cos(PI * x) - 1.0) / 2.0))
   
# Quad
static func easeInQuad(x: float, offset: float=0, length: float=1) -> float:
   x -= offset
   x /= length
   return (0.0 if x < 0 else (1.0 if x > 1.0 else x * x))

static func easeOutQuad(x: float, offset: float=0, length: float=1) -> float:
   x -= offset
   x /= length
   return (0.0 if x < 0 else (1.0 if x > 1.0 else 1.0 - (1.0 - x) * (1.0 - x)))
   
static func easeInOutQuad(x: float, offset: float=0, length: float=1) -> float:
   x -= offset
   x /= length
   return (0.0 if x < 0 else (1.0 if x > 1.0 else (2.0 * x * x if x < 0.5 else 1 - pow(-2 * x + 2, 2) / 2.0)))
   
# Poly
static func easeInPoly(x: float, offset: float=0, length: float=1, poly: float=2) -> float:
   x -= offset
   x /= length
   return (0.0 if x < 0 else (1.0 if x > 1.0 else pow(x, poly)))

static func easeOutPoly(x: float, offset: float=0, length: float=1, poly: float=2) -> float:
   x -= offset
   x /= length
   return (0.0 if x < 0 else (1.0 if x > 1.0 else 1.0 - pow(1.0 - x, poly)))
   
static func easeInOutPoly(x: float, offset: float=0, length: float=1, poly: float=2) -> float:
   x -= offset
   x /= length
   return (0.0 if x < 0 else (1.0 if x > 1.0 else (pow(2.0, poly - 1) * pow(x, poly) if x < 0.5 else 1 - pow(-2.0 * x + 2, poly) / 2.0)))

# Expo
static func easeInExpo(x: float, offset: float=0, length: float=1) -> float:
   x -= offset
   x /= length
   return (0.0 if x < 0 else (1.0 if x > 1.0 else pow(2.0, 10 * x - 10)))

static func easeOutExpo(x: float, offset: float=0, length: float=1) -> float:
   x -= offset
   x /= length
   return (0.0 if x < 0 else (1.0 if x > 1.0 else 1.0 - pow(2.0, -10 * x)))
   
static func easeInOutExpo(x: float, offset: float=0, length: float=1) -> float:
   x -= offset
   x /= length
   return (0.0 if x < 0 else (1.0 if x > 1.0 else (pow(2.0, 20 * x - 10) / 2 if x < 0.5 else (2.0 - pow(2.0, -20 * x + 10)) / 2.0)))
   
# Circ
static func easeInCirc(x: float, offset: float=0, length: float=1) -> float:
   x -= offset
   x /= length
   return (0.0 if x < 0 else (1.0 if x > 1.0 else 1.0 - sqrt(1.0 - pow(x, 2))))

static func easeOutCirc(x: float, offset: float=0, length: float=1) -> float:
   x -= offset
   x /= length
   return (0.0 if x < 0 else (1.0 if x > 1.0 else sqrt(1.0 - pow(x - 1.0, 2))))
   
static func easeInOutCirc(x: float, offset: float=0, length: float=1) -> float:
   x -= offset
   x /= length
   return (0.0 if x < 0 else (1.0 if x > 1.0 else ((1.0 - sqrt(1.0 - pow(2 * x, 2))) / 2.0 if x < 0.5 else (sqrt(1.0 - pow(-2.0 * x + 2, 2)) + 1.0) / 2.0)))
   
# Back
static func easeInBack(x: float, offset: float=0, length: float=1) -> float:
   x -= offset
   x /= length
   var c1 = 1.70158
   var c3 = c1 + 1.0
...
static func easeOutBack(x: float, offset: float=0, length: float=1) -> float:
   x -= offset
   x /= length
   var c1 = 1.70158
   var c3 = c1 + 1.0
...
static func easeInOutBack(x: float, offset: float=0, length: float=1) -> float:
   x -= offset
   x /= length
   var c1 = 1.70158
   var c2 = c1 * 1.525
...
# Elastic
static func easeInElastic(x: float, offset: float=0, length: float=1, wobble: float=10) -> float:
...
static func easeOutElastic(x: float, offset: float=0, length: float=1, wobble: float=10) -> float:
   x -= offset
   var length_trans = 10 / length
   var c4 = (2 * PI) / 3
   return (0.0 if x < 0 else (1.0 if x > length else pow(2, -length_trans * x) * sin((x * wobble - 0.75) * c4) + 1))

static func easeInOutElastic(x: float, offset: float=0, length: float=1) -> float:
   x -= offset
   x /= length
   var c5 = (2 * PI) / 4.5
...
# Bounce
static func easeInBounce(x: float, offset: float=0, length: float=1) -> float:
   var x_trans = x - offset - length
   var x_new = 0.0 if x_trans > 0.0 else abs(x_trans)
   var ease_inv = easeOutBounce(x_new, 0.0, length)
   return -ease_inv + 1.0

static func easeOutBounce(x: float, offset: float=0, length: float=1) -> float:
	x -= offset
	x /= length
	var n1 := 7.5625
	var d1 := 2.75
...
static func easeInOutBounce(x: float, offset: float=0, length: float=1) -> float:
   x -= offset
   x /= length
